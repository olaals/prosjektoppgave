...
camMat = readmatrix('matrices/cam-mat.csv')

intrinsics = cameraIntrinsics([camMat(1,1), camMat(2,2)], principalPoint, imageSize);
...
img_left = imread('images/stereo-images/leftCam.png');
img_right = imread('images/stereo-images/rightCam.png');
...
%figure
%imshow(imgRightRect)
imwrite(imgLeftRect, 'images/rectified/img-left-rect.png');
imwrite(imgRightRect, 'images/rectified/img-right-rect.png');

imgLeftRectGray = rgb2gray(imgLeftRect);
imgRightRectGray = rgb2gray(imgRightRect);
%imshow(imgLeftRectGray)

disparityRange = [128-16, 256-16];
...
writematrix(rgbxyz, 'pointclouds/proj.txt', 'Delimiter', ';');
