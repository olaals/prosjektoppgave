...
def create_pattern_img(patterns, rows):
    img = np.zeros((rows, rows))
    divisions = 2 ** (patterns+1)
    step = rows / divisions
    period = 2 * step
    img = ones((rows, rows)) * ((indices((rows, rows))[1] % period) > step) * 255
...
if __name__ == '__main__':
    ROWS = 1024
    PATTERNS = 7
    SAVE_FOLDER = os.path.join(os.getcwd(), get_argument_or_default("--output", "patterns"))
    PATTERN_FILE_NAME = "pattern_inv"
    EXTENSION = ".png"

    ind = 0
    for i in range(PATTERNS):
        pattern_img = create_inverse_pattern_img(i, ROWS)
        filepath = os.path.join(SAVE_FOLDER, PATTERN_FILE_NAME + "-" +str(ind) + EXTENSION)
